...
El equipo ha definido una arquitectura de despliegue híbrida y distribuida, optimizada para la escalabilidad y seguridad de cada componente de la solución. El proceso de publicación se automatiza mediante tuberías de CI/CD que vinculan los repositorios de GitHub con sus respectivos entornos de ejecución:

*   Landing Page y Frontend Web Applications: Estos productos se despliegan en la plataforma Vercel. El flujo de publicación se activa automáticamente tras cada merge a la rama main, aprovechando las capacidades de Edge Computing de Vercel para garantizar baja latencia y alta disponibilidad global.
*   Web Services: El backend se aloja en un servidor con NixOS, garantizando una configuración de sistema declarativa e inmutable. Para la exposición segura de los servicios, se implementa un Cloudflare Tunnel, eliminando la necesidad de abrir puertos públicos. La gestión de la red privada y el acceso administrativo se realiza mediante Tailscale, asegurando una comunicación encriptada punto a punto entre los nodos de la infraestructura.
*   Edge & Embedded Applications: Los componentes embebidos y dispositivos de borde se gestionan a través de la plataforma Azure IoT. El despliegue de software en estos dispositivos se realiza mediante IoT Hub, permitiendo actualizaciones remotas (OTA), monitoreo de estado y telemetría en tiempo real bajo los estándares de seguridad de Azure.
*   C4 Model: Deployment Diagram



El diagrama adjunto ilustra la topología de red descrita, representando la interacción entre el tráfico de usuario a través de Cloudflare, el entorno de ejecución de las aplicaciones web en Vercel, la infraestructura privada sobre NixOS y la capa de dispositivos gestionada por Azure IoT.
